...
@php
// Memecah nama tim tugas untuk dinamisasi tanda tangan dan paraf tim DLH
...
<div x-data="bapTokoModern()" class="min-h-screen bg-slate-50 font-sans pb-24 relative">
    ...
    <div class="sticky top-0 z-40 bg-white border-b border-slate-200 px-4 py-4 shadow-sm">
        <div class="flex items-center gap-3 mb-3">
            ...
            <div class="flex-1">
                <h2 class="text-base font-black text-slate-800 leading-tight">BAP Toko Modern & Pasar</h2>
    ...
    <form action="/pengawasan/ba/simpan/toko-modern" method="POST" enctype="multipart/form-data" class="p-4" id="formBap">
        @csrf
        ...
        <div x-cloak x-show="step === 1" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-x-4">
            ...
            <div class="bg-indigo-50 border border-indigo-100 p-4 rounded-xl mb-6">
                ...
                <div class="bg-white p-3 rounded-lg text-sm border border-indigo-100 shadow-sm">
                    <p class="font-bold text-slate-800">{{ $agenda->tim_tugas ?? 'Nama Petugas Otomatis' }}</p>
                    <p class="text-slate-500 text-xs">Tim Pengawas Lingkungan Hidup | DLH Kab. Sragen</p>
                </div>
            </div>

            <h3 class="font-bold text-slate-700 mb-4 border-l-4 border-teal-500 pl-3">A & B. Identitas Usaha/Kegiatan</h3>

            <div class="space-y-4">
                <div class="bg-white p-4 rounded-xl border border-slate-200 shadow-sm space-y-4">
                    <div>
                        <label class="block text-[10px] font-bold text-slate-500 uppercase mb-1">Nama Toko Modern / Pasar</label>
                        <input type="text" value="Supermarket / Pasar X (Otomatis dari Agenda)" readonly class="w-full px-3 py-2 rounded-lg border border-slate-200 text-sm bg-slate-100 mb-2">
                        ...
                        <textarea name="bap[alamat]" rows="2" placeholder="Alamat lengkap lokasi..." class="w-full px-3 py-2 rounded-lg border border-slate-200 text-sm focus:border-teal-500"></textarea>
                    </div>
                    ...
                    <div>
                        <label class="block text-[10px] font-bold text-slate-500 uppercase mb-1">Penanggung Jawab Usaha</label>
                        <div class="flex gap-2">
                            <input type="text" name="bap[pj_nama]" placeholder="Nama" class="w-2/3 px-3 py-2 rounded-lg border border-slate-200 text-sm focus:border-teal-500">
                ...
                <div class="bg-white p-4 rounded-xl border border-slate-200 shadow-sm space-y-4">
                    ...
                    <label class="block text-[10px] font-bold text-slate-500 uppercase mt-4">Luas Area (Total & Terbangun)</label>
                    <div class="grid grid-cols-2 gap-3">
            ...
            <div class="space-y-4">
                ...
                <div class="bg-white p-4 rounded-xl border border-slate-200 shadow-sm space-y-4">
                    <div>
                        ...
                        <input type="text" name="bap[kapasitas_kegiatan]" placeholder="Kapasitas (Jumlah Kios/Los/Area Belanja)..." class="w-full px-3 py-2 rounded-lg border border-slate-200 text-sm focus:border-teal-500">
                    </div>
                    ...
                    <div class="grid grid-cols-1 gap-3">
                        <textarea name="bap[shift_kerja]" rows="2" placeholder="Jam Operasional / Jam Shift Kerja / Hari..." class="w-full px-3 py-2 rounded-lg border border-slate-200 text-sm focus:border-teal-500"></textarea>
                        <input type="number" name="bap[hari_kerja_minggu]" placeholder="Jumlah hari buka dalam 1 minggu..." class="w-full px-3 py-2 rounded-lg border border-slate-200 text-sm focus:border-teal-500">
                    </div>
                </div>

                <div class="bg-white p-4 rounded-xl border border-slate-200 shadow-sm space-y-4">
                    <label class="block text-[10px] font-bold text-slate-500 uppercase mb-1">Pengelolaan Air & Limbah Cair</label>
                    <textarea name="bap[teknologi_air_limbah]" rows="2" placeholder="Teknologi Pengelolaan Pembuangan Air Limbah (Misal: Saluran tertutup, Septictank)..." class="w-full px-3 py-2 rounded-lg border border-slate-200 text-sm mb-2 focus:border-teal-500"></textarea>
                    ...
                    <textarea name="bap[penggunaan_energi]" rows="2" placeholder="Penggunaan Energi (Listrik PLN, Genset dll)..." class="w-full px-3 py-2 rounded-lg border border-slate-200 text-sm focus:border-teal-500"></textarea>
                </div>

                <div class="bg-white p-4 rounded-xl border border-slate-200 shadow-sm space-y-4">
                    <h4 class="text-xs font-bold text-slate-600 border-b border-slate-200 pb-2">Dokumen Perizinan</h4>
                    <textarea name="bap[dokumen_dimiliki]" rows="2" placeholder="Dokumen yang dimiliki (SPPL, NIB, dll)..." class="w-full px-3 py-2 rounded-lg border border-slate-200 text-sm focus:border-teal-500"></textarea>
        ...
        <div x-cloak x-show="step === 3" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-x-4">
            <h3 class="font-bold text-slate-700 mb-4 border-l-4 border-amber-500 pl-3">C. Hasil Pengawasan</h3>

            <div class="bg-amber-50 border border-amber-200 p-3 rounded-xl mb-4 flex items-start gap-3">
                <i data-lucide="info" class="w-5 h-5 text-amber-500 mt-0.5"></i>
                <div>
                    <p class="text-xs font-bold text-amber-800">Bab 1. Persetujuan Lingkungan</p>
                    <p class="text-[10px] text-amber-700">Data ditarik otomatis dari database perizinan. Lanjut ke Bab 2.</p>
                </div>
            </div>

            <div class="space-y-6">
                <template x-for="(kategori, catIndex) in checklistTokoModern" :key="catIndex">
        ...
        <div x-cloak x-show="step === 4" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-x-4">
            ...
            <div class="bg-white p-4 rounded-xl border border-slate-200 shadow-sm mb-6">
                <h4 class="text-xs font-bold text-slate-500 uppercase tracking-widest mb-4">Bab 5. Dokumentasi Lapangan</h4>
                
                <div class="p-6 border-2 border-dashed border-indigo-300 rounded-xl bg-indigo-50 text-center mb-4">
                    <input type="file" @change="fotoTemuan.push({file: $event.target.files[0], desc: ''})" 
                           accept="image/*" capture="environment" class="hidden" id="cameraInputMain">
                    <label for="cameraInputMain" class="cursor-pointer block">
                        <i data-lucide="camera" class="w-10 h-10 text-indigo-500 mx-auto mb-2"></i>
                        <p class="text-sm font-bold text-slate-700">Buka Kamera Lapangan</p>
                    </label>
                </div>

                <div class="space-y-4">
                    <template x-for="(item, index) in fotoTemuan" :key="index">
                        <div class="p-3 rounded-xl border border-slate-200 bg-slate-50 relative">
                            <button type="button" @click="fotoTemuan.splice(index, 1)" class="absolute -top-2 -right-2 bg-rose-500 text-white rounded-full p-1.5 shadow-md">
                                <i data-lucide="x" class="w-4 h-4"></i>
                            </button>
                            <img :src="URL.createObjectURL(item.file)" class="w-full h-48 object-cover rounded-lg mb-3 shadow-sm border border-slate-200">
                            <input type="text" :name="'dokumentasi['+index+'][keterangan]'" x-model="item.desc" placeholder="Tulis keterangan foto (Misal: TPS Sampah Domestik)..." class="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg">
                        </div>
                    </template>
                </div>
            </div>
            ...
            <div class="bg-white p-4 rounded-xl border border-slate-200 shadow-sm mb-6">
                <h4 class="text-xs font-bold text-slate-500 uppercase tracking-widest mb-4">Informasi Tambahan & Saran</h4>

                <label class="block text-[10px] font-bold text-slate-500 uppercase mb-1">Riwayat Ketaatan</label>
                <textarea name="bap[riwayat_ketaatan]" rows="2" class="w-full px-3 py-2 rounded-lg border border-slate-200 text-sm mb-3 focus:border-indigo-500" placeholder="Jelaskan riwayat ketaatan pengelola toko/pasar..."></textarea>
            ...
            <div class="bg-white p-5 rounded-xl border border-slate-200 shadow-sm mb-6">
                ...
                <div class="space-y-3 mb-6">
                    <template x-for="(komponen, index) in komponenPenilaian" :key="index">
                        <div class="flex flex-col md:flex-row md:items-center gap-3 p-3 bg-white border border-slate-200 rounded-xl hover:border-indigo-300 transition-colors shadow-sm">
                            ...
                            <div class="flex-1">
                                <!-- PERBAIKAN: name diubah ke rincian_skoring -->
                                <input type="text" x-model="komponen.nama" :name="`rincian_skoring[${index}][nama]`" 
                                       class="w-full bg-transparent border-0 border-b border-dashed border-slate-300 focus:border-indigo-500 focus:ring-0 px-0 py-1 text-sm font-bold text-slate-700" placeholder="Ketik rincian (Misal: Pengelolaan Limbah)...">
                            </div>
                ...
                <!-- PERBAIKAN: name diubah tanpa bap[] -->
        ...
        <div x-cloak x-show="step === 5" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-x-4">
            ...
            <div class="bg-white p-4 rounded-xl border border-slate-200 shadow-sm mb-6">
                <div class="mb-4">
                    <span class="text-xs font-bold text-slate-500 uppercase tracking-widest">Pihak Fasilitas / Usaha</span>
    ...
    <div class="fixed bottom-0 left-0 w-full bg-white border-t border-slate-200 p-3 flex justify-between gap-3 z-50">
        ...
        <button type="submit" form="formBap" x-show="step === 5" class="flex-1 py-3 rounded-xl font-bold text-sm text-white bg-indigo-600 shadow-lg shadow-indigo-500/30 flex justify-center items-center gap-2">
            <i data-lucide="save" class="w-4 h-4"></i> Simpan BAP
        </button>
    </div>
...
<script>
    const templateData = @json($templateTokoModern);

    function bapTokoModern() {
        return {
            step: 1,
            koordinat: '',
            checklistTokoModern: templateData,
            
            // Komponen Tambahan yang sebelumnya missing
            fotoTemuan: [],
            listSaran: [{ id: Date.now() }],
            komponenPenilaian: [{nama: '', nilai: null}],

            // State untuk Signature
            padTtd: null,
            padParaf: null,
            padsTim: [], 
            padsParafTim: [], 
            ...
            getLocation() {
                if (navigator.geolocation) {
                    navigator.geolocation.getCurrentPosition(
                        (position) => {
                            this.koordinat = position.coords.latitude.toFixed(6) + ', ' + position.coords.longitude.toFixed(6);
                        },
...
<ADDITIONAL_METADATA>
The current local time is: 2026-07-15T20:25:20+07:00.
</ADDITIONAL_METADATA>