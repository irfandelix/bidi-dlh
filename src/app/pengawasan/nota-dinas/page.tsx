...
import { ArrowLeft, FileText, Send, Building2, User, Plus, X, Upload } from 'lucide-react';
import LottieLoader from '@/components/LottieLoader';

export default function NotaDinasPengawasanPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const res = await fetch('/api/pengawasan/generate-surat', {
        method: 'POST',
      ...
      let filename = 'Nota_Dinas_Pengawasan.docx';
    ...
    <div className="min-h-screen bg-slate-50 p-4 md:p-8">
      <div className="max-w-4xl mx-auto space-y-6 pb-20">
        ...
        <div className="flex items-center gap-4 mb-6">
          <Link href="/pengawasan" className="w-12 h-12 shrink-0 bg-white border border-slate-200 text-slate-700 rounded-xl flex items-center justify-center shadow-sm hover:bg-slate-50 transition-all">
            <ArrowLeft size={24} />
          </Link>
          <div>
            <h1 className="text-2xl md:text-3xl font-black text-slate-900 tracking-tight uppercase">Buat Nota Dinas</h1>
            <p className="text-sm font-bold text-slate-500 uppercase tracking-wide">Modul Pengawasan</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-3xl shadow-lg border border-slate-200 overflow-hidden">
          <div className="bg-fuchsia-600 p-6 text-white flex items-center gap-4">
            <FileText size={32} />
            <div>
              <h2 className="text-xl font-black uppercase tracking-widest">Formulir Isian Surat</h2>
              <p className="text-fuchsia-200 font-medium text-sm">Akan otomatis meng-generate urutan nomor terbaru</p>
            </div>
          </div>
          ...
          <div className="p-6 md:p-8 space-y-8">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" name="hal" required placeholder="Contoh: Laporan Hasil Pengawasan..." className="w-full bg-slate-50 border-2 border-slate-200 rounded-xl px-4 py-3 text-sm font-bold text-slate-900 focus:border-fuchsia-500 focus:ring-4 focus:ring-fuchsia-100 outline-none transition-all" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase mb-2">Yth. Tujuan</label>
                  <input type="text" name="yth" required defaultValue="Kepala Dinas Lingkungan Hidup Kab. Sragen" className="w-full bg-slate-50 border-2 border-slate-200 rounded-xl px-4 py-3 text-sm font-bold text-slate-900 focus:border-fuchsia-500 focus:ring-4 focus:ring-fuchsia-100 outline-none transition-all" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase mb-2">Dari</label>
                  <input type="text" name="dari" required defaultValue="Tim Pengawas Lingkungan Hidup" className="w-full bg-slate-50 border-2 border-slate-200 rounded-xl px-4 py-3 text-sm font-bold text-slate-900 focus:border-fuchsia-500 focus:ring-4 focus:ring-fuchsia-100 outline-none transition-all" />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-bold text-slate-500 uppercase mb-2">Sifat</label>
                    <input type="text" name="sifat" required defaultValue="Biasa" className="w-full bg-slate-50 border-2 border-slate-200 rounded-xl px-4 py-3 text-sm font-bold text-slate-900 focus:border-fuchsia-500 focus:ring-4 focus:ring-fuchsia-100 outline-none transition-all" />
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-slate-500 uppercase mb-2">Lampiran</label>
                    <input type="text" name="lampiran" required defaultValue="1 (Satu) Berkas" className="w-full bg-slate-50 border-2 border-slate-200 rounded-xl px-4 py-3 text-sm font-bold text-slate-900 focus:border-fuchsia-500 focus:ring-4 focus:ring-fuchsia-100 outline-none transition-all" />
                  </div>
                </div>
              </div>
            </div>
            ...
            {/* Tim Petugas */}
            <div className="space-y-4">
              <h3 className="font-black text-slate-900 uppercase tracking-widest border-b-2 border-slate-100 pb-2">Tim Petugas</h3>
                ...
                <div key={idx} className="flex items-center gap-2">
                  <div className="flex-1 relative">
                    ...
                    <input 
                      ...
                      className="w-full bg-slate-50 border-2 border-slate-200 rounded-xl pl-10 pr-4 py-3 text-sm font-bold text-slate-900 focus:border-fuchsia-500 outline-none transition-all" 
                    />
                  </div>
              ...
              <button type="button" onClick={addPetugas} className="flex items-center gap-2 text-sm font-black text-fuchsia-600 hover:text-fuchsia-700 bg-fuchsia-50 px-4 py-2 rounded-xl transition-all uppercase tracking-widest">
          ...
          <div className="bg-slate-50 p-6 md:p-8 border-t border-slate-200 flex justify-end">
            <button 
              type="submit" 
              disabled={loading}
              className="px-8 py-4 bg-fuchsia-600 hover:bg-fuchsia-700 disabled:opacity-70 disabled:hover:translate-y-0 text-white font-black text-sm uppercase tracking-widest rounded-xl shadow-lg hover:-translate-y-1 hover:shadow-xl transition-all flex items-center gap-3"
            >
