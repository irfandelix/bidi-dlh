...
// CHECKLIST ASLI PERUMAHAN MILIKMU
$templatePerumahan = [
    [
        'bab' => '2. Pemeriksaan Pengelolaan Air Limbah',
        'items' => [
            ['no' => 'a', 'point' => 'Sumber limbah'],
            ['no' => 'b', 'point' => 'Kondisi fisik septictank dan resapan'],
...
<div x-data="bapPerumahan()" class="min-h-screen bg-slate-50 font-sans pb-24 relative">
    ...
    <div class="sticky top-0 z-40 bg-white border-b border-slate-200 px-4 py-4 shadow-sm">
        <div class="flex items-center gap-3 mb-3">
            ...
            <div class="flex-1">
                <h2 class="text-base font-black text-slate-800 leading-tight">BAP Sektor Perumahan</h2>
    ...
    <form action="/pengawasan/ba/update/{{ $agenda->id }}" method="POST" enctype="multipart/form-data" class="p-4" id="formBap">
        ...
        <div x-cloak x-show="step === 1" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-x-4">
            ...
            <h3 class="font-bold text-slate-700 mb-4 border-l-4 border-teal-500 pl-3">A & B. Identitas Pengembang</h3>

            <div class="space-y-4">
                <div class="bg-white p-4 rounded-xl border border-slate-200 shadow-sm space-y-4">
                    <div>
                        <label class="block text-[10px] font-bold text-slate-500 uppercase mb-1">Nama Perumahan / Pengembang</label>
                        ...
                        <textarea name="bap[alamat]" rows="2" placeholder="Alamat lengkap lokasi perumahan..." class="w-full px-3 py-2 rounded-lg border border-slate-200 text-sm focus:border-teal-500"></textarea>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        ...
                        <div>
                            ...
                            <input type="text" name="bap[status_permodalan]" placeholder="PMDN/Swasta..." class="w-full px-3 py-2 rounded-lg border border-slate-200 text-sm focus:border-teal-500">
                        </div>
                    </div>

                    <div>
                        <label class="block text-[10px] font-bold text-slate-500 uppercase mb-1">Penanggung Jawab Perumahan</label>
                ...
                <div class="bg-white p-4 rounded-xl border border-slate-200 shadow-sm space-y-4">
                    ...
                    <label class="block text-[10px] font-bold text-slate-500 uppercase mt-4">Luas Area (Total & Bangunan)</label>
                    <div class="grid grid-cols-2 gap-3">
                        <input type="text" name="bap[luas_total]" placeholder="Total Lahan..." class="px-3 py-2 border border-slate-200 rounded-lg text-xs">
                        <input type="text" name="bap[luas_bangunan]" placeholder="Lahan Bangunan..." class="px-3 py-2 border border-slate-200 rounded-lg text-xs">
                    </div>
                </div>
            </div>
        </div>
        ...
        <div x-cloak x-show="step === 2" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-x-4">
            <h3 class="font-bold text-slate-700 mb-4 border-l-4 border-teal-500 pl-3">B. Operasional Perumahan</h3>
            
            <div class="space-y-4">
                
                <div class="bg-white p-4 rounded-xl border border-slate-200 shadow-sm space-y-4">
                    <div>
                        <label class="block text-[10px] font-bold text-slate-500 uppercase mb-1">Kapasitas (Jumlah & Tipe Rumah)</label>
                        <div class="grid grid-cols-1 gap-3">
                            <textarea name="bap[kapasitas_izin]" rows="2" placeholder="Sesuai Izin (Misal: 100 Unit Tipe 36)..." class="w-full px-3 py-2 rounded-lg border border-slate-200 text-xs focus:border-teal-500"></textarea>
                            <textarea name="bap[kapasitas_riil]" rows="2" placeholder="Kondisi Riil Terbangun..." class="w-full px-3 py-2 rounded-lg border border-slate-200 text-xs focus:border-teal-500"></textarea>
                        </div>
                    </div>

                    <div>
                        <label class="block text-[10px] font-bold text-slate-500 uppercase mb-1">Pekerja (Konstruksi) & Penghuni</label>
                        <textarea name="bap[jumlah_pekerja]" rows="2" placeholder="Jumlah Pekerja (Fase Konstruksi)..." class="w-full px-3 py-2 rounded-lg border border-slate-200 text-sm mb-2 focus:border-teal-500"></textarea>
                        <textarea name="bap[jumlah_penghuni]" rows="2" placeholder="Jumlah Penghuni (Pasca Konstruksi)..." class="w-full px-3 py-2 rounded-lg border border-slate-200 text-sm mb-2 focus:border-teal-500"></textarea>
                    </div>

                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            <label class="block text-[10px] font-bold text-slate-500 uppercase mb-1">Jam Kerja/Hari</label>
                            <input type="number" name="bap[jam_kerja_hari]" placeholder="Misal: 8" class="w-full px-3 py-2 rounded-lg border border-slate-200 text-xs focus:border-teal-500">
                        </div>
                        <div>
                            <label class="block text-[10px] font-bold text-slate-500 uppercase mb-1">Hari Kerja/Minggu</label>
                            <input type="number" name="bap[hari_kerja_minggu]" placeholder="Misal: 6" class="w-full px-3 py-2 rounded-lg border border-slate-200 text-xs focus:border-teal-500">
                        </div>
                    </div>
                    <textarea name="bap[shift_kerja_konstruksi]" rows="2" placeholder="Jam Shift Kerja untuk Pekerjaan Konstruksi..." class="w-full px-3 py-2 rounded-lg border border-slate-200 text-sm focus:border-teal-500"></textarea>
                </div>

                <div class="bg-white p-4 rounded-xl border border-slate-200 shadow-sm space-y-4">
                    <label class="block text-[10px] font-bold text-slate-500 uppercase mb-1">Pengelolaan Air & Limbah Cair</label>
                    <textarea name="bap[teknologi_air_limbah]" rows="2" placeholder="Teknologi Pengelolaan Air Limbah (Misal: Septictank Individu / IPAL Komunal)..." class="w-full px-3 py-2 rounded-lg border border-slate-200 text-sm mb-2 focus:border-teal-500"></textarea>
                    <input type="text" name="bap[volume_air_limbah]" placeholder="Volume Air Limbah (Liter/hari/rumah)..." class="w-full px-3 py-2 rounded-lg border border-slate-200 text-sm mb-2 focus:border-teal-500">
                    
                    <textarea name="bap[pemanfaatan_air_tanah]" rows="2" placeholder="Pemanfaatan Air Tanah (Sumur dangkal/PDAM)..." class="w-full px-3 py-2 rounded-lg border border-slate-200 text-sm mb-2 focus:border-teal-500"></textarea>
                    <input type="text" name="bap[penggunaan_air_liter]" placeholder="Jumlah Penggunaan Air Dalam Satu Hari (Liter/hari)..." class="w-full px-3 py-2 rounded-lg border border-slate-200 text-sm focus:border-teal-500">
                </div>
        ...
        <div x-cloak x-show="step === 3" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-x-4">
            <h3 class="font-bold text-slate-700 mb-4 border-l-4 border-amber-500 pl-3">C. Hasil Pengawasan Perumahan</h3>
            ...
            <div class="space-y-6">
                <template x-for="(kategori, catIndex) in checklistPerumahan" :key="catIndex">
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3 pl-5">
                                        <div>
                                            ...
                                            <textarea :name="'tabel_c['+catIndex+'][items]['+itemIndex+'][kondisi]'" 
                                                      rows="2" class="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm bg-slate-50 focus:bg-white focus:border-amber-500" placeholder="Ketik hasil pengamatan riil..."></textarea>
                                        </div>
                                        <div>
        ...
        <div x-cloak x-show="step === 4" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-x-4">
                        ...
                        <div class="p-4 border border-indigo-100 rounded-xl bg-indigo-50 relative shadow-sm">
                            ...
                            <div class="space-y-3 pr-8">
                                ...
                                <div>
                                    ...
                                    <input type="text" :name="`dokumentasi[${index}][keterangan]`" placeholder="Contoh: Tampak Septictank Blok A..." class="w-full px-3 py-2 rounded-lg border border-indigo-200 text-sm bg-white">
                                </div>
                            </div>
                        </div>
                    </template>
            ...
            <div class="bg-white p-4 rounded-xl border border-slate-200 shadow-sm mb-6">
                ...
                <textarea name="bap[riwayat_ketaatan]" rows="2" class="w-full px-3 py-2 rounded-lg border border-slate-200 text-sm mb-3 focus:border-indigo-500" placeholder="Jelaskan riwayat ketaatan pengembang..."></textarea>
                        ...
                        <div class="flex flex-col md:flex-row md:items-center gap-3 p-3 bg-white border border-slate-200 rounded-xl hover:border-indigo-300 transition-colors shadow-sm">
                            ...
                            <div class="flex-1">
                                <input type="text" x-model="komponen.nama" :name="`rincian_skoring[${index}][nama]`" 
                                    class="w-full bg-transparent border-0 border-b border-dashed border-slate-300 focus:border-indigo-500 focus:ring-0 px-0 py-1 text-sm font-bold text-slate-700" placeholder="Ketik rincian (Misal: Pengelolaan Sampah)...">
                            </div>
        ...
        <div x-cloak x-show="step === 5" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-x-4">
            ...
            <div class="bg-white p-4 rounded-xl border border-slate-200 shadow-sm mb-6">
                <div class="mb-4">
                    <span class="text-xs font-bold text-slate-500 uppercase tracking-widest">Pihak Pengembang / Perumahan</span>
                    <p class="text-[10px] text-slate-400">Tanda tangan & paraf penanggung jawab.</p>
                </div>
    ...
    <div class="fixed bottom-0 left-0 w-full bg-white border-t border-slate-200 p-3 flex justify-between gap-3 z-50">
        ...
        <button type="submit" form="formBap" x-show="step === 5" class="flex-1 py-3 rounded-xl font-bold text-sm text-white bg-indigo-600 shadow-lg shadow-indigo-500/30 flex justify-center items-center gap-2">
            <i data-lucide="save" class="w-4 h-4"></i> Simpan BAP Perumahan
        </button>
    </div>

</div>

<!-- SCRIPT ALPINE DAN SIGNATURE PAD (FIX PERUMAHAN) -->
<script src="https://cdn.jsdelivr.net/npm/signature_pad@4.1.7/dist/signature_pad.umd.min.js"></script>

<script>
    const templateData = {!! json_encode($templatePerumahan) !!};

    function bapPerumahan() {
        return {
            step: 1,
            koordinat: '{{ $agenda->latitude ? $agenda->latitude.", ".$agenda->longitude : "" }}',
            checklistPerumahan: templateData,
            
            // Variabel Penilaian Ditambahkan
...
<ADDITIONAL_METADATA>
The current local time is: 2026-07-15T20:20:10+07:00.
</ADDITIONAL_METADATA>