    ...
    for (const doc of data) {
      ...
      for (const field of fields) {
        if (doc[field] && typeof doc[field] === 'string') {
          if (doc[field].includes('PERTEK.AL')) {
            updates[field] = doc[field].replace('PERTEK.AL', 'ST.AL');
          }
          if (doc[field].includes('PERTEK.EM')) {
            updates[field] = doc[field].replace('PERTEK.EM', 'ST.EM');
          }
        }
      }
